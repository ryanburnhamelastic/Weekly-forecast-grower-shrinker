...
function getAllAccounts(weeklyTabName) {
  try {
    ...
    // Growers: rows 4-14 (ignore header rows 1, 2, 3)
    // Shrinkers: rows 18-28 (ignore header rows 15, 16, 17)
    const growers = extractAllAccountsFromSection(tab, 4, 14, 'Growers');
    const shrinkers = extractAllAccountsFromSection(tab, 18, 28, 'Shrinkers');
...
function extractAllAccountsFromSection(sheet, startRow, endRow, section) {
  ...
  // Column structure based on user clarification:
  // C: Account Name (starred_account)
  // P: Customer Architect
  // Q: Notes
  // Data rows: 4-14 for Growers, 18-28 for Shrinkers (ignore header rows)

  for (let row = startRow; row <= endRow; row++) {
    const rowData = sheet.getRange(row, 1, 1, 17).getValues()[0];  // Read columns A-Q
...
function saveAccountNote(weeklyTabName, rowNumber, noteText) {
  try {
    ...
    // Validate row number is in expected range (4-14 for growers or 18-28 for shrinkers)
    if (!((rowNumber >= 4 && rowNumber <= 14) || (rowNumber >= 18 && rowNumber <= 28))) {
